...
public class WFX_Demo : MonoBehaviour
{
	...
	void OnMouseDown()
	{
		...
		if(this.GetComponent<Collider>().Raycast(UnityEngine.Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 9999f))
		{
	...
	void Update()
	{
		...
		if(rotateCam)
		{
		    UnityEngine.Camera.main.transform.RotateAround(Vector3.zero, Vector3.up, cameraSpeed*Time.deltaTime);
		}
		if(rotate_m4)
		{
